...
def _create_test_records_one(client):
    ...
    assert response.json()["title"] == new_record["title"]
    assert response.json()["visible"] == new_record["visible"]
    # Create the third record with the token from test_user_1,
    # without visibility
    new_record = {
        "title": "test_record_3",
    }
...
def _check_three_records_user_one(client, check_connection=False):
    """
    Check that test_user_1 has no records
    """
    # Get token from test_user_1
...
def test_all_test():
    ...
    with TestClient(app) as client:
        ...
        # 8. Create three records with the token from test_user_1,
        # one of them without visibility
        _create_test_records_one(client)
        # 9. Check that test_user_1 has three record
        ...
        # 14. Check that test_user_1 has three record
        _check_three_records_user_one(client, check_connection=True)
        # 15. Check that test_user_1 has one record if it is filtered by title
